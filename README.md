...
Benötigt werden Git, Docker mit laufender Linux-Engine, Docker Compose ab Version 2.24.0 sowie ein eigenes Hopsworks-Projekt mit API-Key. Für den nachfolgenden Ablauf ein leeres Projekt verwenden: Vorhandene zusätzliche Daten werden durch Sample-Upserts nicht entfernt und könnten das Training sowie den festen Replay-Zeitpunkt beeinflussen.
...
In der kopierten `.env` den Host, Projektnamen und API-Key des eigenen Hopsworks-Projekts eintragen. Für Replay bleibt der OpenWeather-Key leer. Zugangsdaten nicht ins Repository hochladen.
...
Der Upload kann mehrere Minuten dauern, zeitweise ohne weitere Ausgabe. Auf die Abschlussmeldungen aller drei Feature Groups warten, bevor das Training gestartet wird.
...
Die Ausgabe ist als **Replay** gekennzeichnet und ist keine heutige Live-Vorhersage.

Ein vortrainiertes Modell wird nicht mitgeliefert. Schritt 4 erstellt das Modell und `models/latest.json`, die für Schritt 5 erforderlich sind. Der feste Replay-Zeitpunkt gehört zum unveränderten Sample-Datensatz. Bei Training auf zusätzlichen, neueren Daten kann dieser Zeitpunkt vom Skript abgelehnt werden.
...
| `models/` | Beim Training erzeugter Ausgabeordner für Modell, Metadaten und Trainingsdokumentation |
| `docs/evidence/` | Gesicherter Nachweis der erfolgreichen Live-Inferenz |
...
MLflow speichert seine Daten in einem Docker-Volume. Dieses wird nicht über Git übertragen. Beim Ausführen der Anleitung werden Trainingslauf und Modell neu erzeugt.

## Optional: Live-Sammlung und Live-Inferenz

Die Live-Inferenz wurde am 16.09.2026 erfolgreich getestet: Die frisch abgerufene Temperatur von **16,75 °C** und das 3h-Aggregat von **17,63 °C** wurden gemeinsam verwendet. Das Aggregat lag **14,6 Minuten** hinter der Wetterbeobachtung. Die Vorhersage für **09:47 CEST** betrug **16,11 °C**.

[Live-Nachweis als JSON](docs/evidence/live_prediction_20260916T044756206162Z.json)
...
Alte Beispieldaten allein reichen wegen dieser Aktualitätsprüfungen nicht für Live-Inferenz. Fehlen aktuelle Features im eigenen Hopsworks-Projekt, muss zunächst gesammelt werden. Bei leerem Rohdatenordner sind dafür ungefähr drei Stunden zuzüglich Verarbeitung nötig, sofern die Sammlung ohne relevante Lücken läuft. Vor der Inferenz muss das Training aus Schritt 4 erfolgreich abgeschlossen sein.

## Einschränkungen und aufgetretene Schwierigkeiten

Datenimport, Training mit MLflow, Replay und Live-Inferenz wurden erfolgreich im bestehenden Projekt getestet. Der vollständige Ablauf in einem neuen, leeren Hopsworks-Projekt wurde bisher nicht separat nachgewiesen.
...
- In einem früheren Testlauf betrug der MAE des Modells 6,731 °C gegenüber 3,061 °C der Persistenzreferenz. Das Modell war damit schlechter als die einfache Referenz.
